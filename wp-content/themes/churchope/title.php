...
elseif (is_page() || (is_single() && $post->post_type == 'post') || (is_single() && $post->post_type == Custom_Posts_Type_Gallery::POST_TYPE) || (is_single() && $post->post_type == Custom_Posts_Type_Testimonial::POST_TYPE) || (is_single() && $post->post_type == Custom_Posts_Type_Event::POST_TYPE)) {
...
if($slider !== false && $slider !== '')	
{
	...
	if (!is_404() && !is_search()){
				...
				if( (is_tax() || is_tag() || is_category()) && $current_term && get_tax_meta($current_term->term_id, SHORTNAME . '_tax_slider', true) != ''){
					$sliderAlias = ('revSlider' == get_tax_meta($current_term->term_id, SHORTNAME . '_tax_slider', true))
						? get_tax_meta($current_term->term_id, SHORTNAME . Admin_Theme_Item_Slideshow::SLIDER, true)
						: '';
					;
				} elseif( !is_tax() && !is_tag() && !is_category() && get_post_meta($pid, SHORTNAME . '_post_slider', true) != '') {
					$sliderAlias = ('revSlider' == get_post_meta($pid, SHORTNAME . '_post_slider', true))
						? get_post_meta($pid, SHORTNAME . Admin_Theme_Item_Slideshow::SLIDER, true)
						: '';
				} else {
