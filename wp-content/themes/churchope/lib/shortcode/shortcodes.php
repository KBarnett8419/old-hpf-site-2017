...
function button($atts, $content = null)
{
	...
	if($download) {
		$download_data = ' data-download="1"';
	}
	
	if(preg_match('/simple_button_black$/', $type))
	{
		$class = 'color' . substr($button_color_fon, 1);
		$out = "<a class='" . $type . " " . $class . "'  href='" . $url . "' " . $target . $download_data. "><b style='background-color: ".$button_color_fon.";'></b><span>" . do_shortcode($content) . "</span></a>";
		$out .= '<style>.simple_button_black.'.$class.':hover { background:'.$button_color_fon.';}</style>';
	}
	else
	{
		$out = "<a class='" . $type . " ' style='background-color: " . $button_color_fon . "' href='" . $url . "' " . $target . $download_data . "  ><span>" . do_shortcode($content) . "</span></a>";
	}
